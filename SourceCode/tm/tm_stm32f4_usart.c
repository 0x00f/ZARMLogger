...
void TM_USART_SendByte(USART_TypeDef* USARTx, uint8_t* DataByte)  {
	...
	if (u->Initialized == 0) {
		...
		USART_WAIT(USARTx);
		/* Send data */
		USARTx->DR = (uint16_t)(*DataByte & 0x01FF);
		/* Wait to be ready, buffer empty */
		USART_WAIT(USARTx);
}

void TM_USART_Send16(USART_TypeDef* USARTx, uint16_t* DataArray, uint16_t count) {
	uint16_t i;
	TM_USART_t* u = TM_USART_INT_GetUsart(USARTx);
	/* If we are not initialized */
	if (u->Initialized == 0) {
